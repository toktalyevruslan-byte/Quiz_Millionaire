...
class DatabaseManager:
    ...
    def _ensure_question_samples(self) -> None:
        ...
        if existing_count > 0:
            print(f"DB SEED: Questions table already contains {existing_count} records.")
            if not os.path.exists(questions_path):
                print("DB SEED: 'questions.json' missing, but database is already seeded. Skipping.")
                return
            # If questions.json exists, we might still want to skip to avoid overhead, 
            # or re-seed if the user explicitly wants updates. 
            # The user asked to "skip this step" if already seeded.
            print("DB SEED: Skipping re-seed as data already exists.")
            return

        if not os.path.exists(questions_path):
            print(f"DB SEED ERROR: {questions_path} not found and database is empty!")
            return
